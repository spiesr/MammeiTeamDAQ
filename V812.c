...
{
  basaddr = control_card_reader("V812_BASE_ADDR");

  //Set the channel mask
  addr = basaddr + 0x4A;
  data = control_card_reader("V812_PATERN_INHIBIT");
  CAENVME_WriteCycle(BHandle,addr,&data,cvA32_U_DATA,cvD16);
  
  //Set channel 0 threshold
  /*addr = basaddr + 0x00;
  data = control_card_reader("V812_THRESH_0");
  CAENVME_WriteCycle(BHandle,addr,&data,cvA32_U_DATA,cvD16);*/

  //Set channel 7 threshold
  addr = basaddr + 0x0E;
  data = control_card_reader("V812_THRESH_7");
  CAENVME_WriteCycle(BHandle,addr,&data,cvA32_U_DATA,cvD16);
  
  //Set majority level threshold
  addr = basaddr + 0x48;
  data = control_card_reader("V812_MAJ_THRESH");
  CAENVME_WriteCycle(BHandle,addr,&data,cvA32_U_DATA,cvD16);
  
  //Set output width for bottom half of channels
  addr = basaddr + 0x40;
