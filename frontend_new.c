...
/*#ifdef HAVE_QADC
  ...
  if (argc == 1)
  {
    printf("Error: duration of run not given...\n");
  ...
  sscanf(argv[1],"%d",&duration_minutes);
  ...
  //duration_minutes = control_card_reader("RUN_DURATION");
  duration = duration_minutes*60;
  duration *= 1e6;
  printf("Run will last for %d minutes...",duration_minutes);
...
//  fprintf(data_log,"Start time: %ld\n",getMicrotime());
  ...
  //fprintf(data_log,"\nStart time: %ld\n",start_time);
  //Gather data for as long as specified
  while ((getMicrotime() - start_time) < duration)
  {
    while (1)
    {
